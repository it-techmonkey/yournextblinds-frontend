...
const StarRating = ({ rating }: { rating: number }) => {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        star <= rating ? (
          <Image key={star} src="/icons/Star.svg" alt="" width={19} height={19} className="w-4 h-4" />
        ) : (
          <Image key={star} src="/icons/starEmpty.svg" alt="" width={18} height={18} className="w-4 h-4" />
        )
      ))}
    </div>
  );
      ...
      <div className="flex flex-col gap-6 lg:gap-8">
        ...
        <div ref={scrollRef} className="w-full overflow-x-auto scrollbar-hide">
          <div className="flex gap-4 pl-6 lg:pl-20">
            ...
            <div className="shrink-0 w-6 lg:w-[48px] h-px" aria-hidden="true" />
          </div>
        </div>
      </div>
    </section>
